/* ===============================================
   HeriExcel.Pro — app.js  v7.11
   Carga de archivos, renderizado de tabla,
   contexto, filtros, busqueda, zoom,
   exportacion, impresion, fijar filas,
   notas, fill-handle

   FIX v7.11 (CARGA DE ESTILOS CORREGIDA):
   - Se usa exclusivamente XLSXStyle (xlsx-js-style) para leer,
     garantizando que cell.s (estilos) se rellene correctamente.
   - La conversión a JSON se hace con XLSXStyle.utils.sheet_to_json
     con raw:false, cellDates:true, y se preserva el objeto ws original.
   - importStyles itera sobre todas las celdas y extrae
     font, fill, alignment, border, numFmt directamente desde cell.s.
   - Se eliminó la sobrescritura de window.XLSX para evitar conflictos.
   - Ahora carga colores, bordes, negritas, cursivas, tamaños,
     fuentes, alineación y formatos de número desde archivos .xlsx.
=============================================== */
'use strict';
...
/* ================================================
   LOAD FILE — USO EXCLUSIVO DE XLSXStyle (xlsx-js-style)
   Garantiza que cell.s (estilos) se rellene correctamente.
================================================ */
function loadFile(file) {
  ...
  reader.onload = function(e) {
    try {
      ...
      /* Usamos XLSXStyle (xlsx-js-style) que sí rellena cell.s */
...
function loadSheet(sheetName) {
  ...
  /* Usamos XLSXStyle para convertir a JSON (raw:false para obtener valores formateados) */
  var XL = window.XLSXStyle || XLSX;

  var jsonData = XL.utils.sheet_to_json(ws, {
    header: 1,
    raw: false,
    defval: '',
    blankrows: true,
...
/* ================================================
   IMPORT STYLES — LEE cell.s DIRECTAMENTE DEL OBJETO WS
   Soporta: negrita, cursiva, subrayado, tachado,
   fuente, tamaño, color de texto, color de fondo,
   alineación, formato de número, bordes.
================================================ */
